...
import { IdentificationIcon,PaperAirplaneIcon ,DocumentDuplicateIcon ,MagnifyingGlassIcon,PlusCircleIcon, MapPinIcon, FlagIcon, ClockIcon, CalendarIcon,UserIcon,NoSymbolIcon, PencilIcon,CheckCircleIcon, ExclamationTriangleIcon} from '@heroicons/react/24/outline';
...
useEffect(()=>{
  ...
  form.append('search_value',formData.search_value);
  form.append('status',formData.status);
   axios.post('http://127.0.0.1:8000/api/internship/filter',form,{ headers:{ Authorization:`bearer ${token}`}}).then(res=>{
      ...
      <div className='flex flex-col min-h-[600px] h-auto gap-y-3  border-blue rounded-lg'>
             ...
             <div className='bg-gray-200 rounded-lg w-full !py-2 flex items-center !px-5 gap-x-1'>
                 <div className='text-2xl font-bold uppercase w-1/2 bg-blue h-[100%] flex justify-start items-center gap-x-3'>
                  <span className=' text-slate-500'>{internships.length}</span> 
                ...
                <div className='flex w-full justify-end gap-x-3'>
                    <div className='w-1/3 h-full flex items-center relative'>
                       <input value={formData.search_value} onChange={handleChange} name='search_value'  placeholder='Rechercher...'  className='bg-white rounded-lg text-sm !p-2 w-full border-2 border-gray-300 focus:border-blue-500 focus:outline-none' />
                       <MagnifyingGlassIcon className='text-slate-400 w-5 h-5 absolute right-3 top-1/2 transform -translate-y-1/2' />
                    </div>
                    
                     <select value={formData.status} onChange={handleChange} name="status" id="" className='!p-2 w-1/6 bg-white border-gray-300 outline-0  border-1 rounded-lg  focus:border-blue-500 capitalize' >
                        <option value="all">tous</option>
                        <option value="published">publié</option>
                        <option value="declined">no publié</option>
              ...
              internships.length>0 ?
                  (
                    <>
                      ...
                      <div className={`bg-gray-200  w-full flex flex-col gap-y-5 !p-5 rounded-lg border-1 border-gray-300 min-h-[200px] h-auto ${ internships.length>3 && 'overflow-y-scroll'} `}>
                      ...
                      {/* no applications */}
                          <div className='bg-gray-200 h-auto w-full flex justify-center items-center !p-5 rounded-lg border-1 border-gray-300 min-h-[200px]'>
                              <div className='bg-red w-[50%] h-[200px] flex flex-col items-center gap-y-2'>
                                  <div className='bg-blue-200 h-[70px] w-[70px] rounded-full flex justify-center items-center'><ExclamationTriangleIcon className='w-6 h-6  text-blue-600'/> </div>
                                  <div className='text-lg capitalize font-bold'>Aucune stage trouvée</div>
                                  <p className='text-sm text-center text-slate-700 first-letter:capitalize'>Il n'y a pas encore d'Offre de Stage</p>
                                  <Link className=' w-fit h-[25%] !py-1 !px-2 flex gap-x-3 justify-center items-center bg-blue-600 text-white rounded-lg'>
                                    <PlusCircleIcon className='w-7 h-7 '/> <div className='font-bold text-md capitalize'>créer une nouvelle offre de stage</div>
                                  </Link>
                              </div>                 
